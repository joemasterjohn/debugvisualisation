...
/**
 * This criteria punishes out-bounded nodes with a large value. A node is
...
public class InBounds implements Criteria {
	...
	public double apply(LayoutEntity[] entities,
		...
		for (LayoutEntity e : entities) {
			...
			if (xe < x)
				result += 1000000 + 100 * (x - xe);
			if (xe + we > w)
				result += 1000000 + 100 * (xe + we - w);
			if (ye < y)
				result += 1000000 + 100 * (y - ye);
			if (ye + he > h)
				result += 1000000 + 100 * (ye + he - h);
		}
